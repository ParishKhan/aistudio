...
description: >-
  ...
  picking the right tier saves multiples — a field guide with verified 2026
  rates.
...
Anything that doesn't need an answer in seconds — evals, backfills,
enrichment, report generation — belongs on the batch endpoint at half price.
...
- **Time-of-day pricing**: DeepSeek's official V4 doubles rates during
  Chinese business hours — schedule batch work off-peak.
- **Tokenizer differences**: Claude's newer tokenizer produces ~30% more
  tokens for the same text than older Claude models — compare cost per task,
